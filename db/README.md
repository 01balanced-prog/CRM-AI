...
- `activities.demo`: общее или индивидуальное демо, отправленное в касании.
- `log_touch` пересоздан с параметром `demo`. Старая сигнатура удалена.
  Клиент версии 11 продолжает работать, у параметра есть значение по умолчанию.
- `segments`: справочник сегментов. Существующие значения в `companies.segment`
  приведены к нему («Суши, доставка» → «Суши»). Нераспознанные остаются как есть.
- `scripts.kind` принимает `message`: шаблоны сообщений в WhatsApp.
- Скрипт «Звонок после демо», два возражения из журнала, три сообщения.
  «Снятие брифа» выключается, если админ его не правил.

Применять **до** публикации `index.html` версии 11: клиент передаёт `demo`
в `log_touch` и читает `segments`.

Файлы `08`–`12` выполняются подряд, каждый заканчивается контрольной строкой.
