...
- `log_touch` пересоздан с параметром `demo`. Старая сигнатура удалена, права как в `12`:
  у `public` отобраны, `authenticated` выдан.
  Клиент версии 11 продолжает работать, у параметра есть значение по умолчанию.
- `segments`: справочник сегментов, роли `crm_readonly` выдан `select`. Существующие значения в `companies.segment`
...
## `15_demo_video.sql` — демо это видео

- Тексты из `14`: скрипт «Звонок после демо», возражения «Уже был бот» и «Позвоните,
  когда будет кейс», сообщения «Демо после звонка» и «Напоминание через день»
  переписаны под видеозапись диалога, а не под Агента, которому можно написать.
- Обновляются только строки, которые админ не правил (`updated_by` пуст).
- Схема и права не меняются.

## `16_level.sql` — с кем говорили

- `activities.spoke_with`: администратор, управляющий или собственник. Пусто, если
  разговора не было.
- `log_touch` пересоздан с параметром `spoke_with`. `with_dm` выводится из него:
  управляющий и собственник считаются ЛПР для плана. Старая сигнатура удалена.
- `v_leads` получает колонку `level`: самый высокий уровень по лиду. Цепочка
  `v_stats` → `v_today` → `v_leads` пересобрана, права выданы заново, включая
  `crm_readonly` на `v_leads`.

Применять **до** публикации `index.html` версии 12: клиент передаёт `spoke_with`
и читает `level`.

## `17_readonly_full.sql` — внешнее чтение видит всю базу

- Роль `crm_readonly` получает `select` на оставшиеся таблицы
  (`scripts`, `lessons`, `lesson_progress`, `coaching_notes`, `plan_defaults`,
  `ramp_steps`, `plans`, `custom_pricing`, `invites`) и семь представлений
  (`v_today`, `v_stats`, `v_plan_today`, `v_kpi_day`, `v_kpi_week`,
  `v_kpi_month`, `v_team_today`), плюс `execute` на `today_msk`, `msk_day`,
  `effective_plan` и `plan_streak`.
- Политики `*_ext_read` на девять таблиц — только `for select`.
- Прав на запись не добавляется ни одного. Файл сам себя проверяет: читает
  каждый открытый объект под новой ролью и пробует записать в два из них.
- `v_today`, `v_stats` и `v_plan_today` внешнему ключу данных не дадут: они
  фильтруют по `current_profile_id()`, а у внешнего ключа в токене нет `sub`.
  Те же числа за отдел целиком дают `v_team_today` и `v_kpi_*`.
- Представления не пересоздаются, роль `authenticated` не трогается.
- **Чего в базе нет, то пропускается.** `custom_pricing` есть в
  `00_baseline.sql`, но в живой базе её никогда не заводили: клиент ручные
  цены не использует. Жёсткий список уронил бы файл на первом же таком имени,
  поэтому каждый объект проверяется через `to_regclass`, а пропущенные
  перечисляются в `notice`. Фиксированных чисел в контрольной строке поэтому
  нет — важно, что прав на запись ноль.

Наружу вместе с ключом уходят персональные данные сотрудников: нормы, сдача
уроков, замечания наставника, почта в приглашениях. Это решение владельца.
Ключ хранить как пароль, отзывать через `revoke crm_readonly from authenticator;`.

## `18_fill_contacts.sql` — дозаполнение связи

Достаёт из заметок (`companies.note`, `companies.todo`) и из `contacts.phone_raw`
номер, ссылку Instagram и ссылку 2ГИС и заполняет ими **пустые** поля. Заполненное
не перезаписывает, заметки не чистит, ничего не удаляет.

Нужен потому, что старый разбор вставленной таблицы выбрасывал данные в заметку:
номер вида «8 928 … доб. 5», Instagram вида «@ник», колонку, не разложенную по полям.
Плюс контакт из листа касания мог получить `phone_raw` без `phone_e164`: номер в базе
есть, а карточка показывает «телефона нет».

Пять шагов, отчёт в контрольной строке:

1. `phone_e164` из `phone_raw` там, где он пуст.
2. Основной контакт из номера в заметке — только компаниям без единого номера.
3. Основным помечается контакт с номером, если основного у компании нет: кнопки
   «Позвонить» и WhatsApp в карточке берут номер только из основного.
4. `instagram_url` из ссылки или «Instagram: @ник» в заметке.
5. `gis_url` из ссылки 2ГИС в заметке, в том числе без `https://`.

Номера берутся только мобильные (9xx) и только в разметке 3-3-2-2: в заметке рядом
лежат средний чек, число точек и год, и из них легко собрать номер, которого нет.
Городской мобильным шаблоном не ловится — так и задумано.

Схему не меняет: ни таблиц, ни колонок, ни представлений, ни прав. Функции разбора
временные, в конце файла удаляются. Идемпотентен: второй прогон заполняет ноль строк.

Чего в базе нет совсем, тот файл не придумает. Остаток показан в контрольной строке
(`left_no_phone`, `left_no_instagram`, `left_no_gis`); его закрывает повторная вставка
таблицы на экране загрузки или кнопки «поиск» в карточке.

Файлы `08`–`18` выполняются подряд, каждый заканчивается контрольной строкой.
