...
## `18_settings.sql` — настройки админа

- `stages`: этапы воронки справочником. Название, порядок, видимость, флаг
  «в MRR», какой скрипт открывать. Свои этапы добавляются из «Настроек».
  Восемь старых кодов системные: переименовать можно, удалить нельзя;
  `new`, `no_answer`, `won`, `lost` нельзя и выключить. Этап, который уже
  встречался в журнале, не удаляется — только выключается.
- `lost_reasons`: причины отказа, так же. `unreachable` ставит база.
- Ограничения check на `leads.status` и `leads.lost_reason` сняты (ищутся по
  тексту: имена в живой базе неизвестны), вместо них внешние ключи на справочники.
- `tariffs.active`: старый тариф выключается, сделки на нём остаются.
- `app_settings`: форма касания (блоки, подписи, порядок, чипы дат).
- `v_stats` пересоздана: «MRR в работе» по флагу `stages.in_pipeline`.
  От `v_stats` ничего не зависит, `v_today` и `v_leads` не трогаются.
  Права `authenticated` и `crm_readonly` выданы заново.

Применять **до** публикации `index.html` версии 15. Если опубликовать клиент
раньше, он работает на значениях по умолчанию, а «Настройки» просят миграцию.
